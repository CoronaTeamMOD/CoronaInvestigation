...
const elementarySchoolPlaceType = 'בית ספר יסודי';
const highSchoolPlaceType = 'בית ספר תיכון';
...
const SchoolEventForm : React.FC = () : JSX.Element => {
    ...
    React.useEffect(() => {
        if (placeSubType !== '') {
            ...
            if (placeSubType === elementarySchoolPlaceType) gradesOptions = elementarySchoolGrades;
            else if (placeSubType === highSchoolPlaceType) gradesOptions = highSchoolGrades;
            setInteractionEventDialogData({...interactionEventDialogData, grade: gradesOptions[0]});
