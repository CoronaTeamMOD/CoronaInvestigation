...
import { requiredText, invalidDateText, max10LengthNumErrorMessage } from 'commons/Schema/messages';
...
const ClinicalDetailsSchema = (validationDate: Date, Gender: string) => {
    return yup.object().shape({
        [ClinicalDetailsFields.ISOLATION_ADDRESS]: yup.object().shape({
            ...
            [ClinicalDetailsFields.ISOLATION_APARTMENT]: yup.string().nullable().max(10, max10LengthNumErrorMessage),
            [ClinicalDetailsFields.ISOLATION_HOUSE_NUMBER]: yup.string().nullable().max(10, max10LengthNumErrorMessage)
        }).required(),
