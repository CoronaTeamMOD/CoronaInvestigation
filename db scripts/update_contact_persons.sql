		...
		if identificationNumber is not null then
			if exists (select  1
				...
				where ce.investigation_id = (
						...
						join contacted_person cp on ce.id  = cp.contact_event
						where cp.id = contactedPersonId) and  p.identification_number = identificationNumber and p.identification_number is not null
