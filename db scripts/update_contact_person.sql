CREATE OR REPLACE FUNCTION public.update_contact_person(contacts json, contact_event_id int4, investigationid int4)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
declare
 	...
 	--identificationType
 		if identificationNumber is not null then
	 	 	if exists(select 1
				...
				where investigation_id = investigationid and p.identification_number = identificationNumber
					and p.identification_number is not null) then
	    ...
	    set first_name = firstName,
	    	...
	    	identification_type = (case when identificationNumber is null then null
				 				  	     when identificationType is null then '?"?'
