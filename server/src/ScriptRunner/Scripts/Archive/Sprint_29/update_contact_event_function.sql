...
flightNum varchar;
busCompany varchar;
isThereMoreVerified bool;
detailsAdditionalVerified varchar;
...
events_array int4[];
creationSource int4;
contacts_in_currency_event boolean = false;
begin 
	...
	if contact_events is not null and contact_events::TEXT != '[]' then
		...
		investigationId :=(select value::text::int4 from json_each(input_data) where key='investigationId');
			...
			loop
				...
				select nullif((contact_event->'isThereMoreVerified')::text,'null')::boolean as val into isThereMoreVerified;
				select nullif((contact_event->'detailsAdditionalVerified')::text,'null') as val into detailsAdditionalVerified;
				...
				if curr_event is null then
					...
					is_repetitive,
					is_there_more_verified,
					details_additional_verified,
					allows_hamagen_data,
					start_time,
					end_time,
					place_name,
					...
					values(investigationId,
						isRepetitive,
						isThereMoreVerified,
						trim(detailsAdditionalVerified,'"'),
						allow_hamagen,
						startTime,
						endTime,
						trim(placeName,'"'),
					...
					details_additional_verified=trim(detailsAdditionalVerified,'"'),
				...
				if contacts is not null and areContactsEmpty = false and 
				(isRepetitive = false or (isRepetitive = true and contacts_in_currency_event = false)) then
					perform public.update_contact_person(contacts,curr_event,investigationId);
					events_array := array_append(events_array, curr_event);
					if isRepetitive = true then
						contacts_in_currency_event:=true;
					end if;
				end if;
