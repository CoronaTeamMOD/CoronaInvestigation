...
select id FROM public.user 
		WHERE investigation_group =  county_input;
	...
	SELECT COUNT(epidemiology_number) INTO unallocatedInvestigationsCount FROM filtered_investigations
		WHERE EXISTS (SELECT id FROM public.user 
			WHERE id =creator AND (user_name = 'לא משויך' OR is_active = false)) AND investigation_status IN (1 , 100000002);
